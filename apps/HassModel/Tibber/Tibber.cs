...
namespace NetDaemonApps.apps.HassModel.Tibber;
...
public class TibberNotification {
  ...
  private async Task Get(IHaContext ha) {
    ...
    _ = await client.GetHomeConsumption(homeId, EnergyResolution.Monthly);
    ...
    switch(current.Level) {
      case PriceLevel.VeryCheap:
        ...
        if(DateTime.UtcNow.AddHours(2).Hour is >= 5 and <= 8) {
